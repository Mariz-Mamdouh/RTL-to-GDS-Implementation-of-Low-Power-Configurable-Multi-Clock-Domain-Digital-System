...
# Created by write_sdc on Tue Sep 23 20:35:17 2025
...
group_path -name INOUT  -from [list [get_ports RST] [get_ports REF_CLK] [get_ports UART_CLK] [get_ports UART_RX_IN]]  -to [list [get_ports UART_TX_OUT] [get_ports parity_error] [get_ports framing_error]]
group_path -name INREG  -from [list [get_ports RST] [get_ports REF_CLK] [get_ports UART_CLK] [get_ports UART_RX_IN]]
group_path -name REGOUT  -to [list [get_ports UART_TX_OUT] [get_ports parity_error] [get_ports framing_error]]
...
set_clock_groups -asynchronous -name REF_CLK_1 -group [list [get_clocks REF_CLK] [get_clocks ALU_CLK]] -group [list [get_clocks RX_CLK] [get_clocks TX_CLK] [get_clocks UART_CLK]]
