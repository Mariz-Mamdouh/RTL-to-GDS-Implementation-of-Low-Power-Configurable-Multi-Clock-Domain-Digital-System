...
create_clock -period $SCAN_CLK_PER -name $SCAN_CLK_NAME [get_ports scan_clk]
...
set_clock_uncertainty -setup $CLK_SETUP_SKEW [get_ports {REF_CLK UART_CLK TX_CLK RX_CLK ALU_CLK scan_clk}]
set_clock_uncertainty -hold $CLK_HOLD_SKEW   [get_ports {REF_CLK UART_CLK TX_CLK RX_CLK ALU_CLK scan_clk}]

# 4. Clock Transitions
set_clock_transition -rise $CLK_RISE  [get_clocks "REF_CLK UART_CLK scan_clk"]
set_clock_transition -fall $CLK_FALL  [get_clocks "REF_CLK UART_CLK scan_clk"]
...
set_clock_groups -asynchronous -group [get_clocks "REF_CLK ALU_CLK"] -group [get_clocks "RX_CLK TX_CLK UART_CLK"] -group [get_clock scan_clk]
...
set_input_delay $in_delay_rx   -clock RX_CLK   [get_ports UART_RX_IN]
set_input_delay $in_delay_scan -clock scan_clk [get_ports {SI SE test_mode}]
...
####################################################################################
           ...
           #########################################################
####################################################################################
set_driving_cell -library scmetro_tsmc_cl013g_rvt_ss_1p08v_125c -lib_cell BUFX2M -pin Y [get_ports {UART_RX_IN SI SE test_mode scan_clk scan_rst}]
...
####################################################################################
           ...
           #########################################################
####################################################################################
set_load 0.1 [get_ports {parity_error framing_error UART_TX_OUT SO}]
